...
// Custom WhatsApp SVG Icon since it's not in Lucide by default
...
const IssueDetail = ({ issue, isOpen, onClose, onFlagClick }) => {
  ...
  // Initialize data and lock body scroll when modal opens
  useEffect(() => {
    if (isOpen && issue) {
  ...
  const {
    _id, title, category, status, priority, subCategory, description, location,
    isAnonymous, reportedBy, media, confirmationCount, impactScore, createdAt, isPublic
  } = issue || {};
  ...
  const handleWhatsappShare = () => {
    setLocalShareCount(prev => prev + 1); // Optimistic Update
    incrementShare();
        ...
        {issue && (
          <>
            ...
            <div className="overflow-y-auto w-full no-scrollbar p-4 md:p-6 flex-1">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-4">
                <div className="bg-background border border-border/50 rounded-xl p-4 text-center">
                  <div className="text-2xl font-black text-green-500 mb-1">{confirmationCount || 0}</div>
            ...
            <div className="p-4 md:px-6 border-t border-border/50 bg-background/80 backdrop-blur-xl z-20 rounded-b-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              {/* Dynamic Stats & Social Share Buttons */}
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                <button
                  ...
                  className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap shadow-md hover:shadow-lg ${safeStatus === 'OPEN'
