...
import {
    ...
    History, Zap, Camera, Download, Activity, Trophy, Medal, Star
} from 'lucide-react';
...
const AdminAnalytics = () => {
    ...
    const openCareerModal = async (userId) => {
        try {
            ...
            setCareerModal({
                ...
                history: res.data.data.history || {}, // Ensuring it's an object
                view: 'OVERVIEW',
                selectedCategory: '',
                issueList: []
            });
    ...
    if (statsLoading) return <div className="flex justify-center items-center h-[60vh]"><MiniLoader /></div>;
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 md:space-y-8 flex flex-col min-h-full pb-10">
            ...
            {/* --- SEPARATOR --- */}
            <div className="w-full h-px bg-gradient-to-r from-transparent via-border/50 to-transparent my-4"></div>

            {/* --- BOTTOM ISSUES DATA TABLE --- */}
            <div className="flex flex-col gap-4">
                <h3 className="text-xl font-black text-foreground flex items-center gap-2">
                    <Search className="text-primary" /> Browse All Issues
                </h3>
                ...
                {/* Table */}
                <div className="bg-card/40 backdrop-blur-2xl border border-border/60 rounded-2xl overflow-hidden shadow-xl flex-1 flex flex-col relative z-[10]">
                    <div className="overflow-x-auto thin-scrollbar bg-background/20 min-h-[300px]">
                        {issuesLoading ? (
            ...
            {/* 1. LIST MODAL (Drill-down from Metric Cards) */}
            <AnimatePresence>
                {listModal.isOpen && (
                                        ...
                                        >
                                            {listModal.type === 'USER' ? (
                                                <div className="flex items-center gap-4">
                                                    <img src={getCorsSafeUrl(item.profilePic) || getAvatar(item.name)} alt="pfp" className="w-12 h-12 rounded-full border border-border/50 object-cover" />
                                                    <div>
                                                        <h5 className="font-bold text-foreground group-hover:text-primary transition-colors">{item.name}</h5>
                                                        <p className="text-xs text-muted-foreground mt-0.5">{item.contact?.email || 'No Email'} • <span className="uppercase font-bold text-primary/80">{item.role}</span></p>
                                                    </div>
                                                </div>
                                            ) : (
            ...
            {/* 2. CAREER PAGE MODAL (With Clickable Stats and Dynamic List View) */}
            <AnimatePresence>
                        ...
                        >
                            ...
                            {careerModal.view === 'OVERVIEW' ? (
                                <div className="flex-1 overflow-y-auto thin-scrollbar p-4 md:p-6 bg-background/30">
                                    ...
                                    {/* 🟢 NEW: Leaderboard Rankings History */}
            ...
            <AnimatePresence>
                {issueModal.isOpen && issueModal.issue && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-2 sm:p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setIssueModal({ ...issueModal, isOpen: false })} />
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center p-4 md:p-5 border-b border-border/50 bg-muted/20 shrink-0">
                                ...
                                <button onClick={() => setIssueModal({ ...issueModal, isOpen: false })} className="p-2 rounded-full bg-card border border-border/50 hover:bg-muted transition-colors"><X size={20} /></button>
                            </div>
                            ...
                            <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-y-auto lg:overflow-hidden thin-scrollbar">
                                ...
                                <div className="w-full lg:w-1/2 p-4 md:p-6 lg:border-r border-border/50 flex flex-col gap-5 lg:overflow-y-auto thin-scrollbar bg-background/50">
                                ...
                                <div className="w-full lg:w-1/2 flex flex-col bg-card/40 lg:overflow-y-auto thin-scrollbar relative">
                                    <div className="p-4 md:p-6 border-b border-border/50 bg-background/50">
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            {issueModal.issue.bidding?.winningBid ? (
                                                <div className="bg-indigo-500/5 border border-indigo-500/20 p-4 rounded-2xl shadow-sm">
                                                    ...
                                                    <p className="text-[10px] text-indigo-500/80 font-bold mt-0.5">Comm. Time: {issueModal.issue.bidding.winningBid.commitmentTimeHours} hrs</p>
                                                </div>
                                            ) : (
                                        ...
                                        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-primary/50 before:to-transparent pb-4">
                                            {generateTimeline(issueModal.issue).map((event, i) => (
                                                <div key={i} className="relative flex items-start gap-4">
                                                    <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-background shadow-sm ${event.color} z-10 shrink-0`}>
                                                        {event.icon}
                                                    </div>
...
const MetricCard = ({ icon: Icon, title, count, color, bg, onClick }) => (
    <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        onClick={onClick}
        className="relative overflow-hidden text-left p-5 rounded-2xl bg-card/40 backdrop-blur-xl border border-border/50 shadow-sm transition-all duration-300 hover:shadow-lg group"
    >
        ...
        <div className="relative z-10 flex flex-col h-full justify-between gap-3">
            <div className="flex items-center justify-between">
                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                    <Icon size={14} className={color} /> <span className="truncate">{title}</span>
                </p>
                <div className={`opacity-0 group-hover:opacity-100 transition-opacity ${color}`}><Search size={14} /></div>
            </div>
            <h3 className="text-3xl font-black text-foreground drop-shadow-sm">{count}</h3>
        </div>
    </motion.button>
);
