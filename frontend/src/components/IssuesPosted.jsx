  ...
  if (loading && page === 1) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-muted-foreground">Loading posted issues...</div>
      </div>
    );
  }
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto thin-scrollbar">
            {displayedIssues.map((issue, index) => (
              <div key={issue._id} className="glass-card rounded-lg p-4 flex justify-between items-start hover:shadow-lg transition-all">
                ...
                {/* Issue Image */}
                {issue.image && (
                  <div className="w-24 h-24 rounded-lg bg-gradient-to-br from-primary/10 to-secondary/10 flex-shrink-0 ml-4">
                    <img 
                      src={issue.image} 
                      alt={issue.title}
